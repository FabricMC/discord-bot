...
public final class ActionCommand extends Command {
	private static final DateTimeFormatter dateFormatter = DateTimeFormatter.ISO_LOCAL_DATE.withZone(ZoneOffset.UTC);
	...
	public boolean run(CommandContext context, Map<String, String> arguments) {
		try {
				...
				if (actions.isEmpty()) {
					...
					for (ActionEntry action : actions) {
						...
						sb.append(String.format("\n%d %s: %s%s%s",
								action.id(),
								dateFormatter.format(Instant.ofEpochMilli(action.creationTime())),
								action.type().id,
								duration,
								reason));
					}
			...
			case "get": {
				...
				if (action == null) {
					context.channel().sendMessage("Unknown action");
					return false;
				}
				...
				if (action.expirationTime() < 0) {
					...
					expirationSuffix = String.format("\n**Expiration:** %s",
							ActionUtil.timeFormatter.format(Instant.ofEpochMilli(action.expirationTime())));
				...
				if (action.suspensionTime() < 0) {
					...
					suspensionSuffix = String.format("\n**Suspension:** %s\n**Suspender:** %s%s\n**Eff. Duration:** %s",
							ActionUtil.timeFormatter.format(Instant.ofEpochMilli(action.suspensionTime())),
				...
				for (long targetDiscordId : context.bot().getUserHandler().getDiscordUserIds(action.targetUserId())) {
				...
				context.channel().sendMessage(new EmbedBuilder()
						...
						.setDescription(String.format("**User %d:**%s\n**Type:** %s\n**Status:** %s\n**Moderator:** %s\n**Creation:** %s%s%s%s%s",
								...
								ActionUtil.timeFormatter.format(Instant.ofEpochMilli(action.creationTime())),
