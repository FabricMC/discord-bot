...
public final class FilterCommand extends Command {
	...
	public boolean run(CommandContext context, Map<String, String> arguments) throws Exception {
		...
		case "list": {
			...
			if (filters.isEmpty()) {
				context.send(String.format("No filters in group %s", group));
			} else {
		...
		case "add": {
			...
			context.send("Filter %d added".formatted(id));
		...
		case "import": {
			...
			if (result.totalPatterns() == 0) {
				...
				context.send("%d / %d filters added".formatted(result.newPatterns(), result.totalPatterns()));
			} else {
				context.send("%d / %d filters added, invalid: %s".formatted(result.newPatterns(), result.totalPatterns(), result.invalidPatterns()));
			}
		...
		case "clear": {
			...
			context.bot().getFilterHandler().reloadFilters();
			context.send("%d filters removed".formatted(count));
		...
		case "setpattern": {
			...
			context.bot().getFilterHandler().reloadFilters();
			context.send("Filter pattern updated");
		...
		case "setgroup":
			...
			context.bot().getFilterHandler().reloadFilters();
			context.send("Filter group updated");
